...
export const PlatformPostCanvas = ({ workspace, generatedContent, credits, deductVisualCredits, setIsCreditModalOpen }) => {
  const { showToast, addGlobalAsset, setActiveModule, studioTarget } = useWorkspace();
  ...
  // Automatically trigger 2-Agent image generation when coming from Custom Strategy card with autoGenerate: true
  ...
  useEffect(() => {
    ...
    if (isCustomStrategyCard && contentData.autoGenerate) {
      ...
      if (!autoGeneratedRef.current.has(key)) {
        ...
        console.log('[PlatformPostCanvas] 🚀 Auto-triggering 2-Agent visual generation for Custom Strategy card:', contentData.topic);
        handleGenerateVisual();
      }
    }
  }, [contentData?.topic, contentData?.autoGenerate, contentData?.isCustomStrategy]);
